...
from
    (select s_suppkey, s_nationkey, unnest(s_partsupps) as ps from supplier) sps,
...
having sum(ps.ps_supplycost * ps.ps_availqty) > (
    ...
    from (select s_suppkey, s_nationkey, unnest(s_partsupps) as ps2 from supplier) sps2,
         (select unnest(r_nations) as n2 from region) ns2
