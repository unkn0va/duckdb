with supplier_parts as (
    ...
    from (select s_suppkey, s_acctbal, s_name, s_address, s_phone, s_comment,
                 s_nationkey, unnest(s_partsupps) as ps from '/home/layun03/data/tpch-nested/1/supplier.parquet') sps
),
nations_asia as (
    select n.n_nationkey as n_nationkey, n.n_name as n_name
    from (select r_name, unnest(r_nations) as n from '/home/layun03/data/tpch-nested/1/region.parquet') rn
    where r_name = 'ASIA'
),
min_cost as (
...
from
    supplier_parts sp,
    '/home/layun03/data/tpch-nested/1/part.parquet' p,
    nations_asia na,
    min_cost mc
where
